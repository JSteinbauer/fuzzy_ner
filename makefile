...
build_fuzzy_ner:
	g++ -O3 -c -I`pwd`/edlib/edlib/include/ -L`pwd`/edlib/build/lib/ -std=c++17 fuzzy_ner.cpp -fopenmp -ledlib
...
build_wrapper:
	...
	-I . -I`pwd`/edlib/edlib/include fuzzy_ner_wrapper.cpp -o fuzzy_ner`python3.9-config \
	--extension-suffix` -L. -lfuzzyner -Wl,-rpath,.